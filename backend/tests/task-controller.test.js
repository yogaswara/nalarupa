...
describe('TaskController', () => {
  ...
  describe('generateImage', () => {
    it('should return 202 and task ID on successful image generation request', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ taskId: '123', status: 'pending' });
    });
    ...
    it('should keep concurrent generate requests isolated by visitor ID', async () => {
      ...
      expect(firstRes.json).toHaveBeenCalledWith({ taskId: 'task-visitor-a', status: 'pending' });
      expect(secondRes.json).toHaveBeenCalledWith({ taskId: 'task-visitor-b', status: 'pending' });
    });
  });
  ...
  describe('getTaskStatus', () => {
    it('should return 200 and task details on successful status retrieval', async () => {
      ...
      expect(GetTaskStatusUseCase.prototype.execute).toHaveBeenCalledWith('123');
    ...
    it('should return 404 if task is not found', async () => {
      ...
      expect(GetTaskStatusUseCase.prototype.execute).toHaveBeenCalledWith('nonexistent');
      expect(res.status).toHaveBeenCalledWith(404);
      expect(res.json).toHaveBeenCalledWith({ error: 'Task not found.' });
    });
  });
  ...
  describe('getGallery', () => {
    it('should return 200 and gallery items', async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith(mockGalleryItems);
    });
